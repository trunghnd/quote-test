...
let Deal = class {
    ...
    async load(id) {    
        ...
        let urlContact = base + '/crm/v3/objects/deals/'+id+'?properties='+propList
        let res = await axios.get(urlContact, config)

        this.data = res.data.properties
        for(let i = 0; i<this.essentialProps ;i++){
    ...
    async save() {    
        ...
        let urlContact = base + '/crm/v3/objects/deals/'+this.data.hs_object_id
